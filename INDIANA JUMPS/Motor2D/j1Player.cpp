...
bool j1Player::Start()
{
	...
	// Player hitbox
	playerHitbox = App->collision->AddCollider({ (int)pos_player.x, (int)pos_player.y, 32, 64 }, COLLIDER_PLAYER, (j1Module*)App->entity_m);
...
void j1Player::OnCollision(Collider* c1, Collider* c2) 
{
	...
	{
		// touches from above
		if ((c2->rect.y) > (playerHitbox->rect.y + playerHitbox->rect.h - 10))
		{ 
			from_up = true; 
		}
		// touches from right
		else if ((c2->rect.x) > (playerHitbox->rect.x + playerHitbox->rect.w - 10))
		{ 
			from_right = true; 
		}
		// touches from left
		else if ((c2->rect.x + (c2->rect.w)) < (playerHitbox->rect.x + 10))
		{ 
			from_left = true; 
		}
		// touches from bottom
		else if ((c2->rect.y + (c2->rect.h)) < (playerHitbox->rect.y + 10))
		{
			from_down = true;
		}
	}
